...
# --- merged from tests/test_cli_map.py (converted in-process) ---


class TestMapCommand:
    def test_map_help(self, capsys):
        with pytest.raises(SystemExit) as exc:
            cli.main(["map", "--help"])
    ...
    def test_map_runs_against_empty_cache(self, capsys):
        """In an empty cache, every template is 'never-run' — exit code 1 (or 0)."""
        try:
            cli.main(["map", "--no-color"])
            code = 0
...
# --- merged from tests/test_cli_import.py ---
