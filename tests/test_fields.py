...
def test_field_handler_factory():
    ...
    assert handler.parse("a") is None
    assert handler.parse("123") == 123
    assert handler.parse("123.123") == 123.123
...
def test_field_handler_with_series():
    ...
    assert all(
        parsed[~parsed.isna()].reset_index(drop=True) == pd.Series([123, 123.123])
    )
    ...
    assert parsed.isna().sum() == 1
    assert all(
        parsed[~parsed.isna()].reset_index(drop=True)
        == pd.Series([123, 123.123, 123_123.123])
    )
