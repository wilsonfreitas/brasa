...
def test_fieldset_validate_record():
    ...
    assert fs.validate_record(invalid_record_unknown_field) == {"id": True, "unknown": False}
...
def test_fieldset_to_dict(sample_fields):
    ...
    expected_dict = {
        ...
        'description': 'Test FS',
        'fields': [
            {'name': 'id', 'description': 'Unique ID', 'type': 'integer', 'required': True},
            {'name': 'name', 'description': 'Item Name', 'type': 'string', 'max_length': 50}
        ]
    }
...
def test_fieldset_from_dict_invalid_field_item_type():
    ...
    with pytest.raises(FieldsetError, match="Each field in 'fields' list must be a dictionary."):
        Fieldset.from_dict(data)
