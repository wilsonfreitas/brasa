...
def test_field_creation_invalid_type_definition():
    with pytest.raises(FieldError, match="Invalid type definition for field 'bad_field'"):
...
def test_field_to_dict():
    field = Field("test_field", "Test Description", "numeric(dec=2)", required=True, unit="USD")
    expected_dict = {
        'name': 'test_field',
