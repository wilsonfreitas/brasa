...
def sample_fieldset_for_pandas():
    ...
    fs.add_field(Field("tx_datetime", "Transaction Datetime", "datetime(format='%Y-%m-%d %H:%M:%S')"))
...
def test_pandas_adapter_read_csv_success(sample_fieldset_for_pandas, sample_csv_content_for_pandas):
    adapter = PandasAdapter(sample_fieldset_for_pandas, errors='raise')
    df = adapter.read_csv(io.StringIO(sample_csv_content_for_pandas.splitlines()[0] + '\n' + sample_csv_content_for_pandas.splitlines()[1]))
    ...
    assert pd.api.types.is_datetime64_any_dtype(df['raw_date']) # parse_dates makes it datetime64

def test_pandas_adapter_read_csv_coerce_errors(sample_fieldset_for_pandas, sample_csv_content_for_pandas):
    adapter = PandasAdapter(sample_fieldset_for_pandas, errors='coerce', verbose_warnings=False)
...
def test_pandas_adapter_read_csv_raise_errors(sample_fieldset_for_pandas, sample_csv_content_for_pandas):
    adapter = PandasAdapter(sample_fieldset_for_pandas, errors='raise', verbose_warnings=False)
    with pytest.raises(ValueError, match="Error parsing field 'amount'"):
        adapter.read_csv(io.StringIO(sample_csv_content_for_pandas))

def test_pandas_adapter_read_csv_ignore_errors(sample_fieldset_for_pandas, sample_csv_content_for_pandas):
    adapter = PandasAdapter(sample_fieldset_for_pandas, errors='ignore', verbose_warnings=False)
    df = adapter.read_csv(io.StringIO(sample_csv_content_for_pandas))
...
def test_pandas_adapter_validate_dataframe(sample_fieldset_for_pandas, sample_csv_content_for_pandas):
    adapter = PandasAdapter(sample_fieldset_for_pandas, errors='coerce', verbose_warnings=False)
    df = adapter.read_csv(io.StringIO(sample_csv_content_for_pandas))
    ...
    assert validation_results['amount']['non_null_count'] == 6 # One invalid, one missing
...
def test_pandas_adapter_no_nullable_dtypes(sample_fieldset_for_pandas, sample_csv_content_for_pandas):
    adapter = PandasAdapter(sample_fieldset_for_pandas, use_nullable_dtypes=False, errors='coerce', verbose_warnings=False)
    df = adapter.read_csv(io.StringIO(sample_csv_content_for_pandas))
