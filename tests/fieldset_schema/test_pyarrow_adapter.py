...
def sample_fieldset_for_pyarrow():
    ...
    fs.add_field(Field("tx_datetime", "Transaction Datetime", "datetime(format='%Y-%m-%d %H:%M:%S')")) # Custom datetime
    fs.add_field(Field("tx_time", "Transaction Time", "time(format='%H:%M:%S')")) # Custom time
...
def test_pyarrow_adapter_get_schema(sample_fieldset_for_pyarrow):
    ...
    assert schema.field('amount').type == pa.string() # Preprocessed, so string initially
    assert schema.field('tx_date').type == pa.string() # Preprocessed, so string initially
    assert schema.field('tx_datetime').type == pa.string() # Preprocessed, so string initially
    assert schema.field('tx_time').type == pa.string() # Preprocessed, so string initially
...
def test_pyarrow_adapter_read_csv_pure_pyarrow(simple_fieldset, simple_csv_path):
    ...
    assert table.schema.field('col_float').type == pa.float64() # Default for numeric without dec
...
def test_pyarrow_adapter_read_csv_with_pandas_preprocessing(sample_fieldset_for_pyarrow, sample_csv_content_for_pyarrow):
    adapter = PyArrowAdapter(sample_fieldset_for_pyarrow, errors='coerce', verbose_warnings=False)
    ...
    assert table.schema.field('amount').type == pa.float64()  # Numeric(dec=4) preprocessed
    assert table.schema.field('tx_date').type == pa.date32()  # Custom date format preprocessed
    assert table.schema.field('tx_datetime').type == pa.timestamp('us')  # Custom datetime format preprocessed
    assert table.schema.field('tx_time').type == pa.time64('us')  # Custom time format preprocessed
...
def test_pyarrow_adapter_read_csv_raise_errors(sample_fieldset_for_pyarrow, sample_csv_content_for_pyarrow):
    adapter = PyArrowAdapter(sample_fieldset_for_pyarrow, errors='raise', verbose_warnings=False)
    with pytest.raises(ValueError, match="Error parsing field 'amount'"): # Error from pandas preprocessing
        adapter.read_csv(io.StringIO(sample_csv_content_for_pyarrow))

def test_pyarrow_adapter_read_csv_ignore_errors(sample_fieldset_for_pyarrow, sample_csv_content_for_pyarrow):
    adapter = PyArrowAdapter(sample_fieldset_for_pyarrow, errors='ignore', verbose_warnings=False)
    table = adapter.read_csv(io.StringIO(sample_csv_content_for_pyarrow))
...
def test_pyarrow_adapter_use_decimal_for_numeric_false(sample_fieldset_for_pyarrow, sample_csv_content_for_pyarrow):
    adapter = PyArrowAdapter(sample_fieldset_for_pyarrow, use_decimal_for_numeric=False, verbose_warnings=False)
    table = adapter.read_csv(io.StringIO(sample_csv_content_for_pyarrow))
    
    assert table.schema.field('amount').type == pa.float64() # Should be float64 if decimal is false
