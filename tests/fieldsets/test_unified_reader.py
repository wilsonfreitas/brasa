...
def test_unified_reader_select_engine_with_custom_formats(
    sample_fieldset_for_unified_reader, tmp_path
):
    ...
    file_with_custom.write_text(
        "id,name,amount,tx_date,raw_date\n1,A,100,2023-01-01,2023-01-01\n"
    )
...
def test_unified_reader_read_csv_pandas_return_pandas(
    sample_fieldset_for_unified_reader, sample_csv_content_for_unified_reader
):
    reader = FieldsetReader(
        sample_fieldset_for_unified_reader, errors="coerce", verbose_warnings=False
    )
    df = reader.read_csv(
        io.StringIO(sample_csv_content_for_unified_reader),
        engine="pandas",
        return_type="pandas",
    )
...
def test_unified_reader_read_csv_pyarrow_return_pyarrow(
    sample_fieldset_for_unified_reader, sample_csv_content_for_unified_reader
):
    reader = FieldsetReader(
        sample_fieldset_for_unified_reader, errors="coerce", verbose_warnings=False
    )
    table = reader.read_csv(
        io.StringIO(sample_csv_content_for_unified_reader),
        engine="pyarrow",
        return_type="pyarrow",
    )
...
def test_unified_reader_read_csv_pyarrow_return_pandas(
    sample_fieldset_for_unified_reader, sample_csv_content_for_unified_reader
):
    reader = FieldsetReader(
        sample_fieldset_for_unified_reader, errors="coerce", verbose_warnings=False
...
def test_unified_reader_read_csv_auto_engine_return_pandas(
    sample_fieldset_for_unified_reader, sample_csv_content_for_unified_reader, tmp_path
):
    reader = FieldsetReader(
        sample_fieldset_for_unified_reader, errors="coerce", verbose_warnings=False
    )
...
def test_unified_reader_invalid_return_type():
    ...
    reader = FieldsetReader(fs)
    with pytest.raises(
        ValueError, match="Invalid return_type"
    ):  # This error is actually caught by mypy, but for runtime safety
