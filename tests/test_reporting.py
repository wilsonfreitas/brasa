...
class TestProgressDisplayNormalMode:
    """Tests for ProgressDisplay NORMAL mode (rich progress bar)."""

    def test_progress_bar_reaches_total_regardless_of_show_skipped(self):
        """The bar advances to total/total whether or not show_skipped is set.

        NORMAL mode renders a single progress bar (no per-task symbol
        stream), so show_skipped no longer affects its rendering — it
        still matters for VERBOSE mode (see test_verbose_mode_always_shows_skipped).
        """
        import re
        ...
        for status in (TaskStatus.PASSED, TaskStatus.SKIPPED, TaskStatus.SKIPPED):
            progress.update(
                TaskResult(
                    operation="process",
                    template_name="test",
