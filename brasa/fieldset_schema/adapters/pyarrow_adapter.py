...
class PyArrowAdapter:
    ...
    def __init__(
        ...
        self._target_schema: Optional[pa.Schema] = None  # Schema with target types after preprocessing
    ...
    def _get_pyarrow_type(self, field: Field) -> pa.DataType:
        ...
        if type_name == 'integer':
            ...
            # Use date32 for dates (days since epoch)
            return pa.date32()
            ...
            # Use time64 with microsecond precision
            return pa.time64('us')
        ...
        **kwargs: Any
        ...
        if self.verbose_warnings:
            ...
            if self._needs_pandas_preprocessing:
                print(f"  - Preprocessing with pandas: {len(self._needs_pandas_preprocessing)} fields")
        
        # If we need pandas preprocessing, use hybrid approach
        if self._needs_pandas_preprocessing:
            return self._read_csv_with_pandas_preprocessing(filepath_or_buffer, **kwargs)
        else:
    ...
    def _cast_table_to_schema(self, table: pa.Table) -> pa.Table:
        ...
        target_schema: pa.Schema = self._target_schema if self._target_schema else self._schema
