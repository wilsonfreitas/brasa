...
class Fieldset:
    ...
    def add_field(self, field: Field) -> None:
        ...
        if not isinstance(field, Field):
            raise FieldsetError(
                f"Expected Field instance, got {type(field).__name__}"
            )
    ...
    def get_fields_by_type(self, type_name: str) -> List[Field]:
        ...
        """
        return [
            field for field in self.get_all_fields()
        ...
        if 'fields' in data:
            ...
            for field_data in data['fields']:
                if not isinstance(field_data, dict):
                    raise FieldsetError("Each field in 'fields' list must be a dictionary.")
    ...
    @classmethod
    def from_yaml(cls, yaml_content: Union[str, Dict[str, Any]]) -> 'Fieldset':
        """
        ...
        if isinstance(yaml_content, str):
            try:
                ...
                raise FieldsetError(f"Invalid YAML content: {e}")
        else:
            data = yaml_content
        
        if not isinstance(data, dict):
            raise FieldsetError("YAML content must represent a dictionary for the fieldset.")
        
        return cls.from_dict(data)
    
    @classmethod
    def from_template_fields(cls, template_fields: Any, raw_fields: list = None) -> 'Fieldset':
        """
