...
# %%
tb_di1 = (
    brasa.get_dataset("b3-futures-di1").filter(pc.field("business_days") > 0).to_table()
)
tb_cdi = (
    brasa.get_dataset("bcb-data")
...
tb_v1 = (
    ...
    .append_column(
        "maturity_date",
        pyarrow.array(cal.offset(tb_cdi.column("refdate").to_pylist(), 1)),
    )
...
business_days_standard = np.array(
    [21, 21 * 3, 21 * 6, 252, 252 * 2, 252 * 3, 252 * 5, 252 * 10, 252 * 15, 252 * 20]
)
symbols_standard = pyarrow.array([f"DI1T{d}" for d in business_days_standard])
tables = []
for date in tb_di1_curve.column("refdate").unique():
    rates = (
        tb_di1_curve.filter(pc.field("refdate") == date)
        .column("adjusted_tax")
        .to_numpy()
    )
    terms = (
...
for symbol in tb_di1_curve_standard.column("symbol").unique():
    rates = (
        tb_di1_curve_standard.filter(pc.field("symbol") == symbol)
        .column("adjusted_tax")
        .to_numpy()
    )
    dates = tb_di1_curve_standard.filter(pc.field("symbol") == symbol).column("refdate")
    symbols = tb_di1_curve_standard.filter(pc.field("symbol") == symbol).column(
        "symbol"
    )
    returns = np.concatenate([np.array([np.nan]), np.diff(rates)])
    ta = pyarrow.table(
        [dates, symbols, pyarrow.array(returns)], names=["refdate", "symbol", "returns"]
    )
    tables.append(ta)

# %%
tb_di1_curve_standard_returns = pyarrow.concat_tables(tables).sort_by(
    [("refdate", "ascending"), ("symbol", "ascending")]
)
brasa.write_dataset(
    tb_di1_curve_standard_returns.to_pandas(), "b3-curves-di1-standard-returns"
)
