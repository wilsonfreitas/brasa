...
# %%
tb_dap = (
    brasa.get_dataset("b3-futures-dap").filter(pc.field("business_days") > 0).to_table()
)

# %%

tb_dap_curve = tb_dap.select(
...
for date in tb_dap_curve.column("refdate").unique():
    rates = (
        tb_dap_curve.filter(pc.field("refdate") == date)
        .column("adjusted_tax")
        .to_numpy()
    )
    terms = (
...
# %%
tb_dap_curve_standard = pyarrow.concat_tables(tables).sort_by(
...
for symbol in tb_dap_curve_standard.column("symbol").unique():
    rates = (
        tb_dap_curve_standard.filter(pc.field("symbol") == symbol)
        .column("adjusted_tax")
        .to_numpy()
    )
    dates = tb_dap_curve_standard.filter(pc.field("symbol") == symbol).column("refdate")
    symbols = tb_dap_curve_standard.filter(pc.field("symbol") == symbol).column(
        "symbol"
    )
...
# %%
tb_dap_curve_standard_returns = pyarrow.concat_tables(tables).sort_by(
    [("refdate", "ascending"), ("symbol", "ascending")]
)
brasa.write_dataset(
    tb_dap_curve_standard_returns.to_pandas(), "b3-curves-dap-standard-returns"
)
