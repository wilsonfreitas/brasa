...
df_contracts["maturity_date"] = df_contracts["maturity_code"].apply(
    lambda x: maturity2date(x, cal)
)
df_contracts["business_days"] = cal.bizdays(
    df_contracts["refdate"], cal.following(df_contracts["maturity_date"])
)
df_contracts["adjusted_tax"] = (100000 / df_contracts["settlement_price"]) ** (
    252 / df_contracts["business_days"]
) - 1
...
df_contracts.sort_values(["refdate", "maturity_date"], inplace=True)
df_contracts = df_contracts[
    ["refdate", "symbol", "maturity_date", "settlement_price", "business_days"]
]
...
first_contracts.loc[merged["business_days_x"] <= 1, :] = second_contracts.loc[
    merged["business_days_x"] <= 1, :
]
...
df_contracts["maturity_date"] = df_contracts["maturity_code"].apply(
    lambda x: maturity2date(x, cal, "15th day")
)
df_contracts["business_days"] = cal.bizdays(
    df_contracts["refdate"], cal.following(df_contracts["maturity_date"])
)
