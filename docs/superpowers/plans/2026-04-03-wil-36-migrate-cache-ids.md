# WIL-36: Migrate cache_metadata IDs Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Create a one-time migration script that updates `cache_metadata.id` and `download_trials.cache_id` to match the new `DownloadArgs`-based hash computation from WIL-34.

**Architecture:** A standalone Python script with two modes: dry-run (default, read-only) and apply (`--apply` flag). The apply mode backs up the database, computes old-to-new ID mappings, checks for collisions, and executes all updates in a single SQLite transaction. The script reuses `DownloadArgs` and `generate_checksum_for_template` from `brasa.util`.

**Tech Stack:** Python 3.10+, sqlite3, argparse, shutil, brasa's `DownloadArgs` and `generate_checksum_for_template`.
...
| File | Change |
|------|--------|
| `scripts/migrate_cache_ids.py` | Create new script |

---

## Task 1: Implement migration script

**Files:**
- Create: `scripts/migrate_cache_ids.py`

- [ ] **Step 1: Create the script with imports, argument parsing, and main structure**

Create `scripts/migrate_cache_ids.py` with the complete implementation:
...
```bash
uv run ruff check scripts/migrate_cache_ids.py && uv run ruff format scripts/migrate_cache_ids.py
```

Expected: No errors, file may be reformatted.

- [ ] **Step 3: Run the script in dry-run mode to verify it works**

```bash
uv run python scripts/migrate_cache_ids.py
```

Expected output: A summary table showing templates with rows to migrate, total counts, and the dry-run notice. Should show ~24,366 rows to migrate and 0 collisions.

- [ ] **Step 4: Run the script with --apply to execute the migration**

```bash
uv run python scripts/migrate_cache_ids.py --apply
```

Expected output: Same summary table but ending with `[APPLIED] Migration complete. Backup at ...`. A backup file `meta.db.bak-YYYYMMDD-HHMMSS` should exist.

- [ ] **Step 5: Verify migration by running the script again (idempotency)**

```bash
uv run python scripts/migrate_cache_ids.py
```

Expected output: `Nothing to migrate. All IDs are up to date.`
...
```bash
git add scripts/migrate_cache_ids.py
git commit -m "feat(WIL-36): add cache_metadata ID migration script"
```
...
- **Read-only by default:** The script only writes when `--apply` is passed.
- **Backup:** A timestamped copy of `meta.db` is created before any writes.
- **Collision safety:** If two old IDs would map to the same new ID, the script aborts before any UPDATE.
- **Atomic:** All updates run in a single SQLite transaction — all succeed or all roll back.
- **Idempotent:** Running after migration reports nothing to migrate.
- **No tests needed:** This is a one-time migration utility. Manual verification (Steps 3-6) is sufficient.
