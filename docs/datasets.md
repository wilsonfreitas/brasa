...
| `staging.b3-equities-unadjusted-prices` | Unadjusted OHLC+volume for the spot-market universe, from `input.b3-bvbg086`; the two whole-market gap days patched — **2016-02-29** (BDIN, `cod_bdi=2`) and **2021-06-10** (cotahist). Complements `staging.b3-cotahist` (deeper history) and the adjusted-prices datasets above | refdate, symbol, trade_quantity, traded_contracts, volume, open, low, high, close, average | **canonical** (unadjusted spot-market OHLC+volume) |
